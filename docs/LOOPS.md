...
so both this harness and its Codex sibling
(`codex-python-engineering-harness`) validate loop contracts against one
...
- `.loop/**` and `scripts/loop_*` are denylisted for agent writes by
  `protect_sensitive_files.py` (see `.claude/hooks/protect_sensitive_files.py`
...
`template/scripts/_vendor_loop_schemas/` is a verbatim vendored copy of
